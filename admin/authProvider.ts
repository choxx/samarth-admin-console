...
const authProvider: AuthProvider = {
  ...
  getIdentity: () => {
    ...
    if (localStorage.getItem("userData")) {
      const { user: { user } } = JSON.parse(localStorage.getItem("userData") as string);
      console.log(user, "user from indentuty")
      return Promise.resolve({
        id: "user",
        fullName: user?.fullName || user?.data?.accountName || user?.firstName,
        avatar:
